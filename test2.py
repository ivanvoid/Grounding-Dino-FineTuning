...
def get_metrics(labels, bboxes, pred_boxes,pred_logits,pred_phrases, label_map):    
    true_labels = labels
    pred_labels = pred_phrases

    try:
        _true_labels = [label_map[l] for l in true_labels]

        new_pred_boxes = []
        new_pred_labels= []
        for i, l in enumerate(pred_labels):
            if l in true_labels:
                new_pred_boxes += [pred_boxes[i]]
                new_pred_labels+= [label_map[l]]
        _pred_labels = new_pred_labels #[label_map[l] for l in pred_labels]
    except:
        import pdb;pdb.set_trace()

    true_labels = torch.tensor(_true_labels)
    pred_labels = torch.tensor(_pred_labels)

    cm = get_confusion(true_labels, pred_labels, label_map)
    
    iou_metric = get_iou(labels, bboxes, pred_boxes, pred_phrases)
...
def get_confusion(true_labels, pred_labels, label_map):
    cm = {
        'tp':0,
        'tn':0,
        'fp':0,
    ...
    # # import pdb;pdb.set_trace()
    # # total = len(true_labels), len(pred_labels))
    # cm['tn'] = len(true_labels) + len(pred_labels) - (cm['tp'] + cm['fp'] + cm['fn'])
    
    
    # if cm['tn'] < 0:
    #     import pdb;pdb.set_trace()
    # assert cm['tn'] >= 0, "True Negatives cannot be negative"
    
    cm['total'] = (cm['tp'] + cm['fp'] + cm['fn'] + cm['tn'])
...
def get_iou(labels, bboxes, pred_boxes, pred_phrases):
    output = [0.0]
    # import pdb;pdb.set_trace()
    # for i, true_label in enumerate(labels):
    #     for j, pred_label in enumerate(pred_phrases):
    #         best_iou = 0
    #         if true_label == pred_label:
    #             pred_box = pred_boxes[j].unsqueeze(0).float()
    #             # pred_box = box_convert(boxes=pred_box, in_fmt="cxcywh", out_fmt="xyxy")
    ...
    #             true_box = box_convert(true_box, in_fmt='cxcywh', out_fmt='xyxy')
    #             # true_box = box_cxcywh_to_xyxy(true_box)

    #             curr_iou = ops.box_iou(pred_box, true_box).item()
    #             if best_iou < curr_iou:
    #                 best_iou = curr_iou
    #     output += [best_iou]
    return output

def iou_sanaty_check(image, labels, bboxes, pred_boxes, pred_phrases):
    from PIL import Image;from matplotlib.patches import Rectangle;import time;
    import matplotlib.pyplot as plt; plt.figure();plt.imshow(image);

    for box in bboxes:
        plt.gca().add_patch(Rectangle((box[0], box[1]), box[2]-box[0], box[3]-box[1], facecolor='none', edgecolor='green'))

    for box in pred_boxes:
        # box = box_convert(boxes=box, in_fmt="cxcywh", out_fmt="xyxy")
        plt.gca().add_patch(Rectangle((box[0], box[1]), box[2]-box[0], box[3]-box[1], facecolor='none', edgecolor='red'))

    plt.savefig(f'./debug/test_{time.time()}.png')
...
def process_images(
    ...
    for imagename in tqdm(np.unique(df.image_name.values)):
        ...
        pred_boxes,pred_logits,pred_phrases = predict(model, image, text_prompt, 0.25, 0.15, 'cuda', True)
        ...
        iou_sanaty_check(image_source, labels, bboxes, pred_boxes, pred_phrases)
        
        if len(pred_phrases) > 0:
            metrics = get_metrics(labels, bboxes, pred_boxes, pred_logits, pred_phrases, label_map)
